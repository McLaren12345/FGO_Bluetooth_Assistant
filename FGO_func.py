...
def card(NoblePhantasm_no=1):
    Serial.touch_button(gc.button["AttackButton"])  # 点击attack按钮
    time.sleep(2)
    Serial.touch(gc.position["NoblePhantasmLeftBias"] + (NoblePhantasm_no - 1) * gc.position["NoblePhantasmLeftBias"], gc.position["NoblePhantasmLeftBias"])  # 打手宝具,参数可选1-3号宝具位
